...
using namespace offbynull::grid_graph::grid_graph_diagonal;

namespace {
    TEST(GridGraphDiagonal, ListNodes) {
        auto x = [](auto&& g) {
            auto n = g.get_nodes();
            EXPECT_EQ(
    ...
    TEST(GridGraphDiagonal, ListEdges) {
        auto x = [](auto&& g) {
    ...
    TEST(GridGraphDiagonal, NodesExist) {
        auto x = [](auto&& g) {
    ...
    TEST(GridGraphDiagonal, RightEdgesExist) {
        auto x = [](auto&& g) {
    ...
    TEST(GridGraphDiagonal, DownEdgesExist) {
        auto x = [](auto&& g) {
    ...
    TEST(GridGraphDiagonal, DiagEdgesExist) {
        auto x = [](auto&& g) {
    ...
    TEST(GridGraphDiagonal, GetOutputs) {
        auto x = [](auto&& g) {
            using E = typename std::remove_reference_t<decltype(g)>::E;
            ...
            {
                std::set<E> actual {}; // TODO: I can't use being() and end() within set's constructor to automate this?
                for (auto _e : g.get_outputs( std::pair{ 0u, 1u } )) {
                    actual.insert(_e);
                }
                EXPECT_EQ(
                    actual,
    ...
    TEST(GridGraphDiagonal, GetInputs) {
        auto x = [](auto&& g) {
            using E = typename std::remove_reference_t<decltype(g)>::E;
    ...
    TEST(GridGraphDiagonal, GetOutputDegree) {
        auto x = [](auto&& g) {
            ...
            EXPECT_EQ(g.get_out_degree(std::pair{ 0u, 1u } ), 1);
        };
        x(create_vector<std::string, std::string>(2u, 3u));
        x(create_array<std::string, std::string, 2u, 3u>());
        x(create_small_vector<std::string, std::string, 2u, 3u>(2u, 3u));
    ...
    TEST(GridGraphDiagonal, GetInputDegree) {
        auto x = [](auto&& g) {
            ...
            EXPECT_EQ(g.get_in_degree(std::pair{ 1u, 1u } ), 1);
        };
        x(create_vector<std::string, std::string>(2u, 3u));
        x(create_array<std::string, std::string, 2u, 3u>());
        x(create_small_vector<std::string, std::string, 2u, 3u>(2u, 3u));
