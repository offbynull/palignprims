#ifndef GRID_GRAPH_RIGHT_H
#define GRID_GRAPH_RIGHT_H
...
namespace offbynull::grid_graph::grid_graph_right {
    ...
    public:
        ...
        void update_edge_data(const E& edge, ED&& data) {
            ...
            if (n1_down == n2_down && n1_right + 1u == n2_right) {
                this->edges[to_raw_idx(n1_down, n1_right)].right = std::forward<ED>(data);
            }
        }
        ...
        ED& get_edge_data(const E& edge) {
            ...
            auto [n2_down, n2_right] = edge.second;
            if (n1_down == n2_down && n1_right + 1u == n2_right) {
                return this->edges[to_raw_idx(n1_down, n1_right)].right;
            }
        ...
        auto get_root_nodes() {
            return std::views::iota(0u, right_node_cnt)
                | std::views::transform([](const auto& n_right) { return N { 0u, n_right }; });
        }
        ...
        auto get_leaf_nodes() {
            return std::views::iota(0u, right_node_cnt)
                | std::views::transform([&](const auto& n_right) { return N { down_node_cnt - 1u, n_right }; });
        }
