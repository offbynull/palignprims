...
namespace offbynull::aligner::graphs::middle_sliceable_pairwise_alignment_graph {
     ...
     * is isolated to some portion starting from some arbitrary node and ending at some other arbitrary node that's further down (or the
    ...
    public:
         ...
         *
         * The behavior of this constructor / class is undefined if `new_leaf_node_` isn't reachable from `new_root_node_`.
         *
         * @param g_ Backing graph.
         * @param new_root_node_ Node within `g_` to designate as the root node (nodes before `new_root_node_` are hidden).
        ...
        , node_outgoing_edge_capacity { g.node_outgoing_edge_capacity } {
            if constexpr (debug_mode) {
                if (!g.is_reachable(new_root_node_, new_leaf_node_)) {
    ...
    /**
     * Create @ref offbynull::aligner::graphs::middle_sliceable_pairwise_alignment_graph::middle_sliceable_pairwise_alignment_graph
     * instance, where template parameters are deduced / inferred from arguments passed in.
     *
     * @tparam debug_mode `true` to enable debugging logic, `false` otherwise.
     * @return New @ref offbynull::aligner::graphs::middle_sliceable_pairwise_alignment_graph::middle_sliceable_pairwise_alignment_graph
     *     instance.
     */
    template<
        bool debug_mode
    >
    auto create_middle_sliceable_pairwise_alignment_graph(
        ...
        return middle_sliceable_pairwise_alignment_graph<debug_mode, G> { g, new_root_node, new_leaf_node };
    }
}
#endif //OFFBYNULL_ALIGNER_GRAPHS_MIDDLE_SLICEABLE_PAIRWISE_ALIGNMENT_GRAPH_H
