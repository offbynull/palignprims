...
namespace offbynull::aligner::graphs::pairwise_overlap_alignment_graph {
    ...
    PACK_STRUCT_START
    /**
     * Edge ID type for @ref offbynull::aligner::graphs::pairwise_overlap_alignment_graph::pairwise_overlap_alignment_graph.
     *
    ...
    // A scorer that translates calls from the grid_graph backing the pairwise_overlap_alignment_graph, such that the edge type becomes the
    // edge type of pairwise_overlap_alignment_graph.
    /**
    ...
    public:
         ...
         *
         * @param edge Edge.
         * @param down_elem_ref_opt Downward element associated with `edge`, if any.
         * @param right_elem_ref_opt Rightward element associated with `edge`, if any.
    ...
    public:
        ...
        auto get_edges() const {
            ...
            auto real_range {
                g.get_edges()
                | std::views::transform([&](const edge<INDEX>& e) {
        ...
        auto get_outputs_full(const N& n) const {
            ...
            bool has_freeride_to_leaf { n.down == grid_down_cnt - 1u && n.right < grid_right_cnt - 1u };
        ...
        auto get_inputs_full(const N& n) const {
            ...
            bool has_freeride_from_root { n.down > 0u && n.right == 0u };
            blankable_bidirectional_view freeride_set_2 {
                has_freeride_from_root,  // passthru if condition is met, otherwise blank
    ...
    auto create_pairwise_overlap_alignment_graph(
        ...
        using WEIGHT_1 = decltype(
            substitution_scorer(
                std::declval<const overlap_edge<INDEX>&>(),
