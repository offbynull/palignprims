...
namespace {
    ...
    struct graph_bundle {
        ...
        > backing_g;
        suffix_sliceable_pairwise_alignment_graph<is_debug_mode(), decltype(backing_g)> suffix_g;
    ...
    TEST(OAGSuffixSliceablePairwiseAlignmentGraphTest, PrintBasic) {
        ...
        for (auto&& n : g.row_nodes(0zu)) {
            std::cout << n << " ";
        }
        std::cout << std::endl;

        std::cout << "norm1" << std::endl;
        for (auto&& n : g.row_nodes(1zu)) {
            std::cout << n << " ";
        }
        std::cout << std::endl;
    ...
    TEST(OAGSuffixSliceablePairwiseAlignmentGraphTest, CreateViaFactory) {
        ...
        auto g2 {
            create_suffix_sliceable_pairwise_alignment_graph<is_debug_mode()>(
                g_bundle.backing_g,
                { 1zu, 2zu }
            )
        };
