...
#include <cstdint>
#include <stdfloat>
#include "offbynull/aligner/aligners/global_sliced_subdivision_stack_aligner.h"
...
namespace {
    using offbynull::aligner::aligners::global_sliced_subdivision_stack_aligner::global_sliced_subdivision_stack_aligner;
    using offbynull::aligner::scorers::simple_scorer::simple_scorer;
    using offbynull::aligner::aligners::utils::alignment_to_stacked_string;
    using offbynull::utils::is_debug_mode;

    TEST(OAAGlobalSlicedSubdivisionStackAlignerTest, SanityTest) {
        global_sliced_subdivision_stack_aligner<is_debug_mode()> aligner {};
        ...
        const auto& [alignment, score] {
            aligner.align<10zu, 10zu>(down, right, substitution_scorer, gap_scorer, 0zu)
        };
        EXPECT_EQ(4zu, score);
    ...
    TEST(OAAGlobalSlicedSubdivisionStackAlignerTest, ParameterizationTest) {
        global_sliced_subdivision_stack_aligner<is_debug_mode()> aligner {};
        ...
        auto align_ {
            [&]<typename N_INDEX, typename WEIGHT>() {
                ...
                const auto& [alignment, score] {
                    aligner.align_strict<N_INDEX, WEIGHT, 10zu, 10zu>(
                        down,
                        right,
                        substitution_scorer,
                        gap_scorer,
