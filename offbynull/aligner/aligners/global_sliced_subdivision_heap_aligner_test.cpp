...
#include <stdfloat>
#include <cstdint>
#include "offbynull/aligner/aligners/global_sliced_subdivision_heap_aligner.h"
...
namespace {
    using offbynull::aligner::aligners::global_sliced_subdivision_heap_aligner::global_sliced_subdivision_heap_aligner;
    using offbynull::aligner::scorers::simple_scorer::simple_scorer;
    using offbynull::aligner::aligners::utils::alignment_to_stacked_string;
    using offbynull::utils::is_debug_mode;

    TEST(OAAGlobalSlicedSubdivisionHeapAlignerTest, SanityTest) {
    ...
    TEST(OAAGlobalSlicedSubdivisionHeapAlignerTest, ParameterizationTest) {
        global_sliced_subdivision_heap_aligner<is_debug_mode()> aligner {};
        ...
        auto align_ {
            [&]<typename N_INDEX, typename WEIGHT>() {
                ...
                const auto& [alignment, score] {
                    aligner.align_strict<N_INDEX, WEIGHT>(down, right, substitution_scorer, gap_scorer, static_cast<WEIGHT>(0zu))
                };
                return score;
            }
        };
