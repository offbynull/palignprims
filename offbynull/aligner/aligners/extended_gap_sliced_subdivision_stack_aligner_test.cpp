...
#include <cstdint>
#include <stdfloat>
#include "offbynull/aligner/aligners/extended_gap_sliced_subdivision_stack_aligner.h"
...
namespace {
    using offbynull::aligner::aligners::extended_gap_sliced_subdivision_stack_aligner::extended_gap_sliced_subdivision_stack_aligner;
    using offbynull::aligner::scorers::simple_scorer::simple_scorer;
    using offbynull::aligner::aligners::utils::alignment_to_stacked_string;
    using offbynull::utils::is_debug_mode;

    TEST(OAAExtendedGapSlicedSubdivisionStackAlignerTest, SanityTest) {
        extended_gap_sliced_subdivision_stack_aligner<is_debug_mode()> aligner {};
        ...
        const auto& [alignment, score] {
            aligner.align<10zu, 10zu>(down, right, substitution_scorer, initial_gap_scorer, extended_gap_scorer, freeride_scorer, 0)
        };
        EXPECT_EQ(-15, score);
    ...
    TEST(OAAExtendedGapSlicedSubdivisionStackAlignerTest, ParameterizationTest) {
        extended_gap_sliced_subdivision_stack_aligner<is_debug_mode()> aligner {};
        ...
        auto align_ {
            [&]<typename N_INDEX, typename WEIGHT>() {
                ...
                const auto& [alignment, score] {
                    aligner.align_strict<N_INDEX, WEIGHT, 10zu, 10zu>(down, right, substitution_scorer, initial_gap_scorer,
                            extended_gap_scorer, freeride_scorer, static_cast<WEIGHT>(0))
                };
                return score;
            }
        };
