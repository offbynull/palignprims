...
#include <stdfloat>
#include <cstdint>
#include "offbynull/aligner/aligners/extended_gap_sliced_subdivision_heap_aligner.h"
...
namespace {
    using offbynull::aligner::aligners::extended_gap_sliced_subdivision_heap_aligner::extended_gap_sliced_subdivision_heap_aligner;
    using offbynull::aligner::scorers::simple_scorer::simple_scorer;
    using offbynull::aligner::aligners::utils::alignment_to_stacked_string;
    using offbynull::utils::is_debug_mode;

    TEST(OAAExtendedGapSlicedSubdivisionHeapAlignerTest, SanityTest) {
        extended_gap_sliced_subdivision_heap_aligner<is_debug_mode()> aligner {};
        ...
        const auto& [alignment, score] {
            aligner.align(down, right, substitution_scorer, initial_gap_scorer, extended_gap_scorer, freeride_scorer, 0)
        };
        EXPECT_EQ(-15, score);
    ...
    TEST(OAAExtendedGapSlicedSubdivisionHeapAlignerTest, ParameterizationTest) {
        extended_gap_sliced_subdivision_heap_aligner<is_debug_mode()> aligner {};
            ...
            [&]<typename N_INDEX, typename WEIGHT>() {
                ...
                const auto& [alignment, score] {
                    aligner.align_strict<N_INDEX, WEIGHT>(down, right, substitution_scorer, initial_gap_scorer, extended_gap_scorer,
                            freeride_scorer, static_cast<WEIGHT>(0))
                };
                return score;
            }
        };
