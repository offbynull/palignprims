...
#include <cstdint>
#include <stdfloat>
#include "offbynull/aligner/aligners/rotational_sliced_subdivision_stack_aligner.h"
...
namespace {
    using offbynull::aligner::aligners::rotational_sliced_subdivision_stack_aligner::rotational_sliced_subdivision_stack_aligner;
    using offbynull::aligner::scorers::simple_scorer::simple_scorer;
    using offbynull::aligner::aligners::utils::alignment_to_stacked_string;
    using offbynull::utils::is_debug_mode;

    TEST(OAARotationalSlicedSubdivisionStackAlignerTest, SanityTest) {
        rotational_sliced_subdivision_stack_aligner<is_debug_mode()> aligner {};
        ...
        const auto& [alignment, score] {
            aligner.align<20zu, 10zu>(down, right, substitution_scorer, gap_scorer, freeride_scorer, 0)
        };
        EXPECT_EQ(3, score);
    ...
    TEST(OAARotationalSlicedSubdivisionStackAlignerTest, ParameterizationTest) {
        rotational_sliced_subdivision_stack_aligner<is_debug_mode()> aligner {};
