#ifndef OFFBYNULL_ALIGNER_ALIGNERS_LOCAL_DYNAMIC_PROGRAMMING_ALIGNMENT_TYPE_PARAMETERIZER_H
#define OFFBYNULL_ALIGNER_ALIGNERS_LOCAL_DYNAMIC_PROGRAMMING_ALIGNMENT_TYPE_PARAMETERIZER_H
...
namespace offbynull::aligner::aligners::local_dynamic_programming_alignment_type_parameterizer {
    using offbynull::concepts::widenable_to_size_t;
    using offbynull::utils::narrowest_type_for_indexing;

    /**
     * Parameters for dynamic programming local alignment.
     *
    ...
    struct local_dynamic_programming_alignment_type_parameterizer {
    private:
        ...
        //  * leaf node has the most incoming edges: (down * right - 1) of freerides + up to 3 edges from nodes immediately before leaf.
        //  * all other nodes have incoming edges of at most 4: 3 from nodes immediately preceding and 1 free ride coming from root
        static constexpr std::size_t max_parents {
            std::max(
                (max_index * max_index) + 3zu,
                3zu + 1zu
            )
        };
...
#endif //OFFBYNULL_ALIGNER_ALIGNERS_LOCAL_DYNAMIC_PROGRAMMING_ALIGNMENT_TYPE_PARAMETERIZER_H