...
namespace {
    ...
    TEST(OASZipSequenceTest, SanityTest) {
        iota_sequence<is_debug_mode(), unsigned int> seq1 { 10u, 15u };
        iota_sequence<is_debug_mode(), unsigned int> seq2 { 20u, 24u };
        iota_sequence<is_debug_mode(), unsigned int> seq3 { 30u, 36u };
        auto seq { create_zip_sequence<is_debug_mode()>(seq1, seq2, seq3) };
        static_assert(sequence<decltype(seq)>);
        static_assert(std::is_same_v<decltype(seq), zip_sequence<is_debug_mode(), decltype(seq1), decltype(seq2), decltype(seq3)>>);
