...
namespace {
    ...
    TEST(SliceablePairwiseAlignmentGraphBacktrackerTest, RandomizedTesting) {
        ...
        for (auto i : std::views::iota(0u, 1000u)) {
            ...
            EXPECT_NEAR(expected_weight, actual_weight, 0.001);
            // EXPECT_EQ(expected_path, actual_path);  // may fail -- there may be more than 1 optimal path, in which case this test will fail
        }
    }
}
